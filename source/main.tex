...
\begin{figure}
  \centering
  \includegraphics[width=0.92\textwidth]{gpu_and_me}
  \caption[ ]{A picture of the author with a GPU (NVIDIA GTX980) and a copy of this very manuscript. The trusty, consumer-grade, GTX980 GPU that has accompanied me throughout most of my Ph.D. will grant its owner the computing power of a small CPU cluster for a price of around 600€\footnotemark.}
  \label{fig:gpuandme}
\end{figure}
\footnotetext{Note, however, that a GPU series tends to be discontinued around two years after its release, so the GTX980 is no longer being manufactured at the time of writing. Much more powerful models are available nowadays at similar prices.}
...
\ctparttext{An overview of the UAMMD infrastructure.\newline\newline\includegraphics[width=1.0\linewidth]{gfx/titleimage}}
\part{UAMMD: Design and components}\label{pt:uammd}

\cleardoublepage\chapter{The Fermation of subproblems}
...
\cleardoublepage\chapter{It's in the name}\label{ch:design}
...
\cleardoublepage\chapter{Initial remarks and overview}
...
\cleardoublepage\chapter{Building an UAMMD module}
...
\cleardoublepage\chapter{ParticleData}\label{sec:particledata}
...
\cleardoublepage\chapter{A self-contained UAMMD code example}
...
\cleardoublepage\chapter{Particle interactions}\label{sec:interactions}
...
\cleardoublepage\chapter{Long range interactions}\label{sec:nbody}
...
\cleardoublepage\chapter{Short range interactions}\label{sec:shortrange}
...
\cleardoublepage\chapter{Neighbour Lists}\label{sec:nlist}
...
\cleardoublepage\chapter{Bonded Interactions}\label{sec:bonded}
...
\cleardoublepage\chapter{Particle Dynamics}\label{sec:dynamics}
...
\cleardoublepage\chapter{Molecular Dynamics}\label{sec:md}
...
\cleardoublepage\chapter{Langevin Dynamics}\label{sec:langevin}
\glsreset{FPE}
\glsreset{SDE}
...
\cleardoublepage\chapter{Dissipative Particle Dynamics}\label{ch:dpdchap}
\glsreset{LD}
...
\newpage
%\cleardoublepage\chapter{Smoothed Particle Hydrodynamics}
%\todo{Describe SPH}
%

\cleardoublepage\chapter{Brownian Dynamics}\label{sec:bd}
...
\cleardoublepage\chapter{Hydrodynamics in open boundaries}
...
\newpage
\cleardoublepage\chapter{Triply periodic hydrodynamics}
...
\cleardoublepage\chapter{The Immersed Boundary Method (IBM) kernels}\label{sec:ibm}
...
\item Spreading (as defined in Eq. \eqref{eq:spreadoper}) for a discrete set of points:
  ...
  \end{equation}
\item Interpolation (as defined in Eq. \eqref{eq:interpdiscreteJ}) from a discrete set of points:
  \begin{equation}
...
\cleardoublepage\chapter{Hydrodynamics under quasi two-dimensional confinement}\label{sec:q2D}
\begin{figure}[h]
  \centering
...
\cleardoublepage\chapter{Hydrodynamics in doubly periodic geometries}\label{sec:dpstokes}
...
\cleardoublepage\chapter{Triply Periodic Electrostatics} \label{ch:tppoisson}
...
\cleardoublepage\chapter{Doubly Periodic Electrostatics}\label{ch:dppoisson}
\newcommand{\botl}{{0}}
\newcommand{\topl}{{H}}
...
%\end{itemize}
%
%\cleardoublepage\chapter{Radial Distribution Function}
%\cleardoublepage\chapter{Mean Square Displacement}
%\cleardoublepage\chapter{Fast Correlations}
%\cleardoublepage\chapter{Fast Histogram}
%\cleardoublepage\chapter{FFT}
%
%
%\cleardoublepage\chapter{Superpunto}
%\begin{figure}[H]
%  \label{fig:spunto}
%  \centering
...
\cleardoublepage\chapter{Measuring intracellular viscosity}
...
\cleardoublepage\chapter{Star Polymer dynamics in shear flow}\label{ch:starpolymers}
\begin{figure}[H]
  \centering
...
\cleardoublepage\chapter{Hydrodynamics in confined geometries}
...
%\cleardoublepage\chapter{Simulations of dynamically cross-linked actin networks}
%Simulations in this work are accelerated by \uammd's neighbour list capabilities.
%\todo{FILL}

\cleardoublepage\chapter{Optofluidic control of the dispersion of nanoscale dumbbells}
...
\cleardoublepage\chapter{Ongoing work, future directions and conclusions}\label{ch:conclusions}
...
\cleardoublepage\chapter{Spanish conclusions}
...
%
%
%%\cleardoublepage\chapter{Diffusion under soft confinement}\label{ch:softq2D}
...
\cleardoublepage\chapter{Basic notions of CUDA/C++ programming}\label{sec:cpp}
...
In addition to inheritance, C++ offers another way of writing generic code called templates. A template is a function or object that is generic for any type (sometimes the type is restricted under some assumptions). For instance, let us rewrite the function \mintinline{\ucpp}{getAreaOf} in example \ref{code:templates} using metaprogramming instead of inheritance.

\begin{code2}[Using templates to process many types of polygons with the same function.] {label=code:templates}
...
\cleardoublepage\chapter{Dealing with the FFT in the GPU} \label{ch:appendixa}
...
\end{code2}
\subsection*{Discrete differential operators and Nyquist points}
One important detail to keep in mind here is that we must be careful when applying differential operators in Fourier space in the discrete setting.

In particular, while we know that $\fou{\nabla} = i\vec{k}$ in a continuum setting, this is not the case when using the Discrete Fourier Transform.

If we define the DFT as
$$\fou{y}_k = \sum_{i=0}^{N-1} y_n\exp\left(-\frac{2i\pi n}{N}k\right)$$
By using finite differences for the derivative and working with this definition we can see that the coefficients for the derivative are equal to multiplying by $ik$ in each direction, \textbf{except} for the $N/2$ mode (only when N is even), which must be subtracted. In other words, we just set to zero the $N/2$ mode of the derivative.
Funny enough, the second derivative is not equivalent to applying this rule twice, so that $\fou{\nabla}^2 = -k^2$ for every mode.
Here is a nice reference explaining this: \url{https://math.mit.edu/~stevenj/fft-deriv.pdf}

This has consequences, for instance, when implementing the \gls{FCM}. In that case the Greens function for the Stokes equation in Fourier space is
$$\fou{\oper{G}}(\vec{k}) = \frac{1}{8\pi\eta k^2}\left(\mathbb{I} - \frac{\vec{k}\otimes\vec{k}}{k^2}\right)$$
The issue is that this actually comes from the Fourier transform of
$$\oper{G} = -\nabla^2\left(\mathbb{I} + \nabla\nabla^{-2}\nabla\right)$$
in a continuum setting. So in order to apply $\fou{\oper{G}}$ one must take into account the discrete version of $\fou{\nabla}$.



\cleardoublepage\chapter{Boundary Value Problem (BVP) Solver} \label{sec:bvp}
...
%\cleardoublepage\chapter{The Fast Chebyshev Transform}\label{sec:fct}
%
%\begin{itemize}
...
\cleardoublepage\chapter{UAMMD's online documentation}\label{ch:online}
...
\cleardoublepage\chapter{The Transverser and Potential interfaces}\label{ch:transverser}
...
\cleardoublepage\chapter{A full UAMMD simulation example}\label{ch:fullexample}
...
\cleardoublepage\chapter{List of currently implemented modules}\label{sec:modulelist}
...
\section*{Colophon}
You can find the \LaTeX\  source for this manuscript (along with the most recent pdf version) at \url{https://github.com/RaulPPelaez/tesis} or the QR code below.
\begin{figure*}[htpb]
  \centering
  \includesvg[width=0.3\columnwidth]{gfx/tesisqr}
  \includesvg[width=0.3\columnwidth]{gfx/uammdqr}
  \captionsetup{labelformat=empty}
  \caption*{QR codes to the source of this manuscript and the UAMMD repository.}
\end{figure*}
\newline
The author acknowledges the NVIDIA corporation for their GPU donations, some of which were used in the publications that resulted from this Ph.D.
\newline
\begin{center}
  Disclaimer\\

  Only some GPUs were harmed during the development of this work.
\end{center}
\bigskip
