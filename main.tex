...
During the following chapters, we will discuss in detail this conceptual and programmatic hierarchy and follow its branches down to the leaves. This final objects will encapsulate the actual algorithms and physics in the form of isolated, individual modules. In general, these modules will communicate only via the closed loop described by \emph{ParticleData}-\emph{Integrator}-\emph{Interactor} in fig. \ref{fig:uammdsketch}.

One of the main unwritten lessons we will glimpse at while descending the tree is that it is never a good idea to introduce dependencies that climb up through the tree. Just to give some examples of this:
\begin{itemize}
...
Modularity is another central philosophy in \uammd. We understand a module as a piece of code whose internal complexity is much larger than the information it shares with other modules.
...
Be it an \emph{Integrator} (hydrodynamics, molecular dynamics,...) or \emph{Interactor} (short range, bonded,...), all modules in \uammd can be created and stored as individual shared pointers\footnote{A C++ specific kind of reference. See Appendix \ref{sec:cpp} for basic notions of C++.}. For instance, throughout this manuscript, we will typically create a module named ``Module'' using a code similar to the one in code \ref{code:module}.
\begin{code}
\begin{minted}{\ucpp}
#include<uammd.cuh>
...
CUDA/C++ is a compiled language, meaning that before executing a source code, it must be transformed into an executable binary by an external program called the \emph{compiler}. We will also refrain from discussing the compilation of CUDA codes\footnote{\uammds online resources shed light into this process, which might change over time for reasons outside of the author control.}.
