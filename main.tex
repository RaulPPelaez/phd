...
\begin{algorithm}[H]
  \caption{Precomputing the kernel and accessing it via linear indexes. See alg. \ref{visitneigh}}
...
Finally, several works exploring UAMMD's facet as an external, GPUI-enabled, accelerator library are ongoing. Mostly making use of the hydrodynamic solvers, electrostatic solvers and neighbour list capabilities of UAMMD.
...
\begin{itemize}
\item Although this thesis is not centered in new physics per-se, we have discussed several works presenting new physics. Some of these lines of work are still active and more publications are expected.
\end{itemize}
...
\todo{Transverser comes from traverse and transform}
...
\item \mintinline[breaklines]{\ucpp}{Compute compute(real4 position_i, real4 position_j,Info info_i, Info info_j);}
  ...
  characterised by position and info this function must return the
...
\begin{code2}[An example \emph{Potential} that computes Lennard-Jonnes forces, energies and/or virials.]{label=code:potential}
...
\todo{Emphasize details about this example, Someone familiar
     ...
     with an integrator. In other words, you should emphasise that
