...
\item A no-slip  wall at the bottom of the domain, $z=0$ (unbounded at $z>0$). $z\in [0, \infty)$.
  The no-slip \gls{BC} for the wall is simply
  \begin{equation}
    \vec{\fvel}_{DP}(z=0) = 0    
  \end{equation} 
\item A slit channel, with no-slip walls located at $z=0$ and $z=H$. $z\in [0, H]$. Besides the \gls{BC} for the bottom wall, we add a similar one for the top ones
  \begin{equation}
    \vec{\fvel}_{DP}(z=H) = 0
  \end{equation}
\end{enumerate}
...
\begin{equation}
  ...
  \begin{aligned}
  &\vec{\ppos}_i^{\text{img}_b} = \vec{\ppos}_i - 2r_z\hat{\vec{e}}_z, \quad \text{For the bottom wall}\\
...
\end{aligned}
\end{equation}
Solved in a domain with $z\in [0,\infty)$ in the case of a bottom wall and $z\in [0, H]$ in the case of a slit channel. Finally, we set slip \glspl{BC} at the walls so

\begin{equation}
  \label{eq:dpstokescorrbcs}
  \begin{aligned}
    &\vec{\fvel}_{\corr}(z=0) = -\vec{\fvel}_{DP}(z=0)\\
...
\begin{equation}
  ...
  \partial_z^2\fou{p}(\vec{k}, z) - k^2\fou{p}(\vec{k}, z) = ik_x\fou{f}^x + ik_y\fou{f}^y + \partial_z \fou{f}^z
\end{equation}
Where the wave vector $\vec{k}:=(k_x, k_y)$ is defined on the plane and has modulus $k^2 = k_x^2 + k_y^2$. The forces, $\vec{f}=(f^x, f^y, f^z)$ can be non-zero only inside a certain domain with $z\in [0,H]$. We can use this to solve the pressure outside the domain, so that
\begin{equation}
  \partial_z^2\fou{p}(\vec{k}, z) - k^2\fou{p}(\vec{k}, z) = 0, \quad \text{ for } z \notin [0,H]
\end{equation}
Whose solution is
\begin{equation}
  \fou{p}(\vec{k}, z) = C_1\exp(-kz) + C_2\exp(kz)
\end{equation}
By using the boundness of the pressure at $|z|\rightarrow\infty$ we can write the solution outside the domain
\begin{equation}
  \begin{aligned}
    &\fou{p}(\vec{k},z\le 0) = C_2\exp(kz)\\
    &\fou{p}(\vec{k},z\ge H) = C_1\exp(-kz)    
  \end{aligned}
\end{equation}
This implies
\begin{equation}
  \label{eq:dpstokespressurebcs}
  \partial_z\fou{p}(\vec{k}, z=0,H) = \pm k \fou{p}(\vec{k}, z=0,H)
\end{equation}
For a given wave number, $k$, the system in eq. \eqref{eq:dpstokespressure} along the \glspl{BC} in eq. \eqref{eq:dpstokespressurebcs} constitute a \gls{BVP} that can be solved in Chebyshev space with the solver described in Appendix \ref{sec:bvp}.
...
\end{aligned}
\end{equation}
Where the domain is here defined in the $[-1, 1]$ range for simplicity. We can later generalize to any size via a simple change in units. In particular by defining $z = z'/H$ (with $z'\in [-H, H]$) and $k = k'H$.
