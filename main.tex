...
We impose that the sources do not overlap the boundaries in the $z$ direction, $f(|z|>H) = 0$, so that the charge density integrates to one inside the slab. Given that the Gaussian is not compactly supported we truncate it at $n_\sigma a \ge 4 a$ to overcome this, ensuring that the integral is at least $99.9\%$ of the charge $q$.

Finally, we solve \eqref{eq:dppoisson} with the following set of \bcs for the potential
\begin{equation}\phi(x,y,z\rightarrow -H^+)=\phi(x,y,z\rightarrow -H^-)\label{eq:dppoissonbcs1}\end{equation}  
\begin{equation}\phi(x,y,z\rightarrow H^-)=\phi(x,y,z\rightarrow H^+)\label{eq:dppoissonbcs2}\end{equation}
And for the electric field 
\begin{equation}\varepsilon_0 \frac{\partial \phi}{\partial z}(x,y,z\rightarrow -H^+)-\varepsilon_b \frac{\partial \phi}{\partial z}(x,y,z\rightarrow -H^-)=-\sigma_b(x,y)\label{eq:dppoissonbcs3}\end{equation}  
\begin{equation}\varepsilon_0 \frac{\partial \phi}{\partial z}(x,y,z\rightarrow H^-)-\varepsilon_t \frac{\partial \phi}{\partial z}(x,y,z\rightarrow H^+)=\sigma_t(x,y)\label{eq:dppoissonbcs4}\end{equation} 
...
\begin{equation}
  ...
  \varepsilon_0 \Delta \phi_{\dpr}(\vec{\fpos}) = -f(\vec{\fpos}) \quad \text{for}\quad |z|<H
\end{equation}
...
We can use the fact that the source charges are contained inside the domain $z\in[-H, H]$ to find the necessary \bcs to constitute a \gls{BVP}, which can then be solved using the solver in Appendix \ref{sec:bvp} (first introduced in chapter \ref{sec:dpstokes} for the Stokes equation). In particular, we can write
\begin{equation}
  \varepsilon_0 \Delta \phi_{\dpr} = 0\quad\text{for}\quad |z|> H.
\end{equation}
...
\begin{equation}
  \fou{\phi}_{\dpr}(\vec{k}, z \ge \pm H) \propto \exp(\mp kz),
\end{equation}
hinting at the following \bcs for eq. \eqref{eq:dppoissonfreespacepot},
\begin{equation}
  \label{eq:dppoissonfreespacebcs}
  (\partial_z \pm k)\fou{\phi}_{\dpr}(\vec{k}, \pm H) = 0, 
\end{equation}
owning to the continuity of $\phi_{\dpr}$ and $\partial_z\phi_{\dpr}$.
...
\begin{equation}
  ...
  \varepsilon_0(\partial_z^2 - k^2)\fou{\phi}_{\dpr}(\vec{k}, z) = -\fou{f}(\vec{k}, z) \quad \text{for}\quad |z|\le H.
\end{equation}
...
\begin{equation}
  ...
  \begin{aligned}
    &\phi_{\corr}(x,y,z\rightarrow \mp H^\pm) - \phi_{\corr}(x,y,z\rightarrow \mp H^\mp) = -m_{\phi}^{b/t}(x,y),\\
    &\varepsilon_0\partial_z\phi_{\corr}(x,y,z\rightarrow \mp H^\pm) - \varepsilon_{b/t}\partial_z\phi_{\corr}(x,y,z\rightarrow \mp H^\mp) = -m_E^{b/t}(x,y).
  \end{aligned}
\end{equation}
Using $\phi = \phi_{\dpr} + \phi_{\corr}$ we can write them in terms of the already computed $\phi_{\dpr}$, 
\begin{equation}
  \begin{aligned}
    &m_{\phi}^{b/t} = \phi_{\dpr}(z\rightarrow \mp H^\pm) - \phi_{\dpr}(z\rightarrow \mp H^\mp) = 0,\\
    &m_E^{b/t} = \varepsilon_0\partial_z\phi_{\dpr}(z\rightarrow \mp H^\pm) - \varepsilon_{b/t}\partial_z\phi_{\dpr}(z\rightarrow \mp H^\mp) + \sigma_{b/t} =\\
    &\qquad=(\varepsilon_0 - \varepsilon_{b/t})\partial_z\phi_{\dpr}(z=\mp H) +\sigma_{b/t}.
  \end{aligned}
\end{equation}
...
\begin{equation}
  \fou{\phi}_{\corr}(\vec{k}, z) = A(\vec{k})\exp(k z) + B(\vec{k})\exp(-kz), \quad \text{for} \quad |z|<H
\end{equation}
...
\begin{equation}
  \varepsilon_0\partial_z ^2\fou{\phi}_{\dpr}(\vec{k}=\vec{0}, z) = -\fou{f}(\vec{0}, z).
\end{equation}
...
%\end{equation}
%The constant $A_0$ can be evaluated by integrating 
...
Once the correction is evaluated for all wavenumbers, we apply the \gls{FCT} to each one, obtaining the Chebyshev coefficients that can then be summed to the uncorrected solution, $\phi_{\dpr}$ to finally get the total potential, $\phi$, via eq. \eqref{eq:dppoissonpotsep}.
