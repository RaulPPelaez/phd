...
\begin{equation}
 \varepsilon_0\Delta\phi=-f=-\sum_{k=1}^Mq_kg(r_k)\label{eq:dppoisson}
\end{equation}
In a doubly periodic domain of width $H$ and size $L_{xy}$ in the plane.
Here $g(r)$ represents a Gaussian source inside the sum that goes over all markers, $M$.
\begin{equation}
  g(r_k)=\sum_x{\frac{1}{\left(2\pi g_w^2\right)^{3/2}}\exp{\left(\frac{-||{\vec{x}-\vec{z}_k}||^2}{2g_w^2}\right)}}
  \label{eq:dppoisson_gaussian}
\end{equation}
Here $x$ represents the sum going, potentially, over all points in space.
