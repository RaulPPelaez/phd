...
\subsection{The Fokker-Planck Formalism}
...
\begin{equation}
  \label{eq:fpeflux}
  \vec{J}(\vec{x}, t) := \vec{D}^{(1)}(\vec{x},t)P - \vec{\partial}_{\vec{x}}\cdot\left[\tens{D}^{(2)}(\vec{x}, t) P\right]
\end{equation}
The first term is called the drift or transport term (being $\vec{D}^{(1)}$ the drift coefficients) and the second one the diffusion term (being $\tens{D}^{(2)}$ the diffusion coefficients). The \gls{FPE} \eqref{eq:fpe} comes from truncating the Kramers-Moyal equation at the second term\cite{Risken2012}\todo{RAFA: argumento para truncar solo a $n=2$?}.
...
\begin{equation}
  \begin{aligned}
    ...
    &= \left\langle \widetilde{\vec{D}}^{(1)} \vec{\partial}_{\vec{x}}f\right\rangle + \left\langle \tens{D}^{(2)}\vec\partial_{\vec{x}}^2f\right\rangle.
  \end{aligned}
\end{equation}
...
\begin{equation}
  \label{eq:fpemean}
  \frac{d\langle\vec{x}\rangle}{dt} = \left\langle \vec{D}^{(1)}\right\rangle = \underbrace{\left\langle \widetilde{\vec{D}}^{(1)}\right\rangle}_{\text{Drift}} + \overbrace{\left\langle \vec{\partial}_{\vec{x}}\cdot\tens{D}^{(2)}\right\rangle}^{\text{Thermal drift}}
\end{equation}
