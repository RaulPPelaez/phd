...
\section{The Immersed Boundary Method (IBM) kernels}
...
For our purposes, this means that we have to estimate numerically the relation between the width of the kernels and the hydrodynamic radius.
...
\begin{equation}
  ...
  \phi_G(r) = \frac{1}{(2\pi\sigma)^{3/2}}\exp\left(\frac{-r^2}{2\sigma^2}\right)
\end{equation}
...
In a serial implementation of spreading, we can simply sum, for each particle, the contribution $\delta_a(\vec{\ppos} - \vec{\fpos})\vec{F}$ to each nearby cell. And in the case of interpolation a similar strategy can be employed, where each particle sums to its quantity the values $\delta_a(\vec{\ppos} - \vec{\fpos})\vec{v}$ from nearby cells. Note that here $\vec{F}$ and $\vec{v}$ can refer to forces and velocities or in general any per particle and per cell quantities.
We can restrict the cells to the minimum by knowing the support, $n_s$, of the kernel.
In a regular grid, we can find the cell of a given particle with position $x \in [-L/2, L/2)$ as $c_x = \floor((x/L + 0.5)n_x)$. From there, it is straight-forward to find the $n_s$ cells in the support at cells $\vec{c} \pm P$ for odd supports (see fig. \ref{fig:ibm2} (top)). It is important to handle even supports separatedly. In this case, a particle at one side of the middle of a cell will need to visit a different set of cells that one at the other side (see fig. \ref{fig:ibm2} (bottom)).
...
Using the strategy in algorithm \ref{alg:visitneigh} instead of the naive three nested loops results in more (cheap) operations, but reduces the operation to a single loop, reduces the logic pressure and facilitates loop unrolling, also giving the compiler more chances for optimization.
\begin{algorithm}[H]
  \label{alg:visitneigh}
  \caption{Visiting the neighbour cells of a particle, $i$, located at $\vec{\ppos}_i \in [0, L)_{\mathbb R ^3}$ }
  \begin{algorithmic}[1]
    \State $\vec{c}_i \gets$ cell of particle $i$
    \If{$n_s$ is even}
    \State $\vec{s} \gets \floor(\vec{\ppos}_i/h) - \vec{c}_i + 0.5$ \Comment{Shift for even supports}
    \Else
    \State $\vec{s} \gets 0$ 
    \EndIf
    \State $\vec{P}_s\gets n_s/2 -\vec{s}$
    \For{ $j=0$ until $n_s^3$}
    \State $i_x \gets \text{mod}(j, n_s)$
    ...
    \State Process interaction between particle $i$ and neighbour cell $\vec{c}_j$
    \EndFor
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H]
  \label{alg:naivespread}
  \caption{Spreading a quantity defined in a group of particles to an Eulerian grid}
  \begin{algorithmic}[1]
    \For{each particle $i$}
    \For{each cell $j$}
    \State Sum contribution of particle $i$ on cell $j$
    \EndFor
    \EndFor
  \end{algorithmic}
\end{algorithm}
\begin{algorithm}[H]
  \label{alg:naiveinterp}
  \caption{Interpolating a quantity defined in a group of particles to an Eulerian grid}
  \begin{algorithmic}[1]
    \For{each particle $i$}
    \For{each cell $j$}
    \State Sum contribution of particle $i$ on cell $j$
...
\begin{equation}
  \label{eq:fcmvel}
  \fou{\vec{u}}(\vec{k}) = \eta^{-1}B(k)\left(\mathbb{I} - \frac{\vec{k}\otimes\vec{k}}{k^2}\right)\left( i\vec{k}\cdot\fou{\mathcal{Z}} + \fou{\vec{f}}\right)
\end{equation}
Where $B(k)$ is a factor in Fourier space, i.e $B := \frac{1}{k^2}$ for the Green's function in eq. \eqref{eq:stokesoperfou}.
...
Note that any smooth, closely-supported kernel can be used here instead of a Gaussian, which will regularize differently the near field hydrodynamics.
