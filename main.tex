...
In another example of these huge simulations, the authors of \cite{fullatombacteria2016} use $100$ million particles to model a chunk of the interior environment of a bacteria for some tens of nanoseconds (around $10^7$ steps).
...
However, this top-down approach is fundamentally flawed. The final product will probably have a lot of internal dependencies, many of which are probably unnecesary. In software this means that the code will be hard to adapt, extend or even more scaryingly, to maintain.

We humans are not good with complex things (which is probably the reason why complex and complicated share the same ethimology) and, as it can be glimpsed throughout \uammd, my solution to this problem is often to either Fermate the problem or remove it altogether.
...
The \uammd framework takes this KISS\footnote{Keep It Simple, Sir} philosophy very seriously. The foundational concepts of \uammd are supported on a handful of (deliberately) vague assumptions which can be summarize in four:
...
\uammd is a \gpu software, which requires special considerations. In particular, given that the \gpu works as a co-processor with its own separated memory space, when allocating or accessing memory we must specify \emph{where} that memory resides. This location can sometimes be subtle when looking at an isolated piece of code. The separation of memory spaces can become a nuissance specially when dealing with quantities that must naturally be accesed from both the CPU and the \gpu\footnote{The following chapters and the code examples therein assume a certain level of familiarity with \gpu programming and the C++ programming language. A reader not familiar with these concepts can start by reading Appendix \ref{sec:cpp}, which introduces basic notions of both C++ and \gpu programming.}.

The \emph{ParticleData} \uammd class attempts to hide the two memory spaces from the user by handling this memory space internally, ensuring that the CPU and \gpu versions of the arrays it provides are synchronized when requested. This is reflected mainly when requesting a particle property from \emph{ParticleData}, which requires to specify both a location (CPU or \gpu) and an usage intention (read, write or both). \emph{ParticleData} can then use this information to provide an up-to-date reference to the data in the correct memory space.
...
#include<uammd.cuh>
//Additional includes when necessary. The definition of the UAMMD structure can be found in the previous example, which code should be included here.
