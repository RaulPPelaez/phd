...
\end{equation}
\begin{itemize}
\item Integrate Fourier form of Oseen in $k_z$.
\item Separate between $k_\perp$ and $k$ parallel.
\item Write expression in a general way for t2D and q2D both in Fourier and real space.
\item Fabricate Greens function by convolving with  the Gaussian just in the z direction (since xy are incorporated via spreading+FFT). This restricts the algorithm to a Gaussian, since this shape is assumed when constructing G.
\item A similar trick as in PSE is used to separate the noise in two terms. Proving that it is right can be done by computing the correlation of the noise and seeing it is equal to the mobility.
\item The algorithm is now identical to FCM with a different kernel.
