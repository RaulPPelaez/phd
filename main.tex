...
\end{minted}
  \caption{The basic outline of a new \emph{Integrator}. Some advanced functionality has been omitted here for simplicity, refer to Appendix \ref{ch:online} for more information. See chapter \ref{sec:particledata} for more information about how to access particle properties. A class inheriting from \emph{Integrator} has access to a series of built-in members, in particular, a \emph{ParticleData} instance is always available under the name ``pd''. Furthermore, a list of \emph{Interactors} is available in the variable ``interactors''.}
  \label{code:integrator}
\end{code}

%\chapter{Particle-based integration methods for simulating complex fluids}
...
As discussed in sec \ref{sec:einstein} we can in general define the drift coefficients in a general way as
\begin{equation}
  \label{eq:bddrift}
...
We start by considering the more general incompressible steady Navier-Stokes equations. Then, we will introduce the action of the particles as a force-density on the fluid and study how it propagates to the rest of the fluid.

We will take the non-inertial regime for the dynamics of both the fluid and the particles, which will allow us to connect it back to \gls{BD} while taking into account hydrodynamic correlations. In section \ref{sec:icm} we consider the fluid inertial regime. We will briefly describe how particle inertia can be reintroduced as a constraint shortly.
