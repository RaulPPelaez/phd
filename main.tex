...
\begin{figure}
  \centering
  \includesvg[width=\columnwidth]{gfx/iterator}
...
\begin{itemize}
\item In C++, \emph{Iterators} are a type of object that \emph{points} to an element in a range of elements (such as a container or pseudo-container) and has the ability to \emph{iterate} through the elements of that range. The canonical example of an iterator is a pointer to the first element of an array. We can use a pointer to iterate through the contents of the array. However, like many things in programming, the internal workings of a certain iterator are irrelevant as long as the iterator presents its expected behavior. In other words, iterators work ``as is''. For instance, a \emph{constant} input iterator works by always returning the same value, regardless of what ``element'' in the ``container'' it is pointing to, as if it was iterating over an infinite sized array filled with the same number. In practice, a constant input iterator would be implemented as simply returning the same number all the time, without requiring any underlying memory to be allocated or present.
  
\item Iterators play a central role in connecting algorithms with containers in C++.
\item Several categories of iterators exists (see fig. \ref{fig:iterator}) depending on the functionality they provide.
\end{itemize}
