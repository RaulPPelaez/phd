...
\end{equation}
The symbol has been changed to enphasize that this mobility is now a configuration-dependent tensor. In particular, eq. \eqref{eq:bdhimobns} yields a pairwise mobility, depending only on the positions of the particles as
\begin{equation}
  \label{eq:bdhimob}
...
One of the usual choices for the mobility tensor is the \gls{RPY} tensor\cite{Rotne1969,Yamakawa1970}, a regularization of the Oseen mobility at short distances that describe the hydrodynamic interaction of two spheres of radius $a$. In particular, integrating eq. \eqref{eq:bdhimob} with the Stokes solution operator in the surface, $S$, of a sphere of radius $a$
\begin{equation}
  \label{eq:rpymobfour}
  \begin{aligned}
    \tens{M}_{ij}^{\textrm{RPY}} &= \frac{\eta^{-1}}{(4\pi a^2)^2} \int_{S_1}dS_1\int_{S_2}dS_2 \int d\vec{k} \exp(i\vec{k}\vec{r})\hat{\oper{L}}(\vec{k})\\
    &= \eta^{-1} \int \frac{\exp(i\vec{k}\vec{r})}{k^2}\left(\frac{sin(ka)}{ka} \right)^2\left(\mathbb{I} - \frac{\vec{k}\otimes\vec{k}}{k^2}\right)d\vec{k}
\end{aligned}
\end{equation}
Now, the free-space \gls{RPY} mobility tensor can be written as
\begin{equation}
  \label{eq:rpy}
...
The classic strategy for computing the square root of the mobility, originally proposed by Ermak\cite{Ermak1978}, is by direct Cholesky factorization. This operation requires $O(N^3)$ operation, rendering this algorithm unsuitable for large numbers of particles (above $10^4$). Additionally, it has $O(N^2)$ storage requirements, since the full mobility matrix has to be stored.
...
In particular, we will solve eq. \eqref{eq:bdhi} using the \gls{RPY} mobility in with periodic boundary conditions. We can express the periodic \gls{RPY} tensor in Fourier space by replacing the integral in eq. \eqref{eq:rpymobfour} by a sum
\begin{equation}
  \tens{M}_{ij}^{\textrm{RPY}}= \frac{1}{\eta dV} \sum_{\vec{k}} \frac{\exp(i\vec{k}\vec{r})}{k^2}\left(\frac{sin(ka)}{ka} \right)^2\left(\mathbb{I} - \frac{\vec{k}\otimes\vec{k}}{k^2}\right)
\end{equation}
