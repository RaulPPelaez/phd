...
\begin{equation}
  k_{\textrm{cut}} = 2\xi\sqrt{-\log_{10}(\epsilon)}
\end{equation}
...
\begin{equation}
r_{\textrm{cut}}^{\textrm{nf}} = \frac{\sqrt{-\log_{10}(\epsilon)}}{\xi}
\end{equation}

Although the cost of computing the near field can sometimes negate the potential gains of using \gls{PSE} over \gls{FCM}, it is important to note that if the near field cut off radius is less than the minimum distance allowed for a pair of particles this part of the algorithm can be skipped altogether. With the exception of the self terms in the mobility matrix, which amounts to. For instance, in the presence of some kind of steric repulsion (a la \gls{LJ} or WCA).
...
Using other kernels beyond the Gaussian in FCM
