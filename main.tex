...
Be it an \emph{Integrator} (hydrodynamics, molecular dynamics,...) or \emph{Interactor} (short range, bonded,...), all modules in \uammd can be created and stored as individual shared pointers\footnote{A C++ specific kind of reference. See Appendix \ref{sec:cpp} for basic notions of C++.}. For instance, throughout this manuscript, we will typically create a module named ``Module'' using the following piece of code
\begin{code}
\begin{minted}{\ucpp}
#include<uammd.cuh>
...
\end{minted}
  \caption{The basic outline of a new \emph{Interactor}. A lot of advanced functionality has been omitted here for simplicity, refer to Appendix \ref{ch:online} for more information.}
...
See chapter \ref{sec:pd} for more information about how to access particle properties.
...
auto createPairForcesWithPotential(UAMMD sim, Potential tr){
  using PF = PairForces<Potential>;
  return std::make_shared<PF> pf(sim.pd, potential);
}
\end{minted}
