...
\item Introduce next part, nobel algorithms and physics.
...
\begin{itemize}
\item We develop an algorithm to solve the Stokes equation (without fluctuations) in a doubly periodic environment.
\item This allows to simulate systems were particles are not restricted to a plane, so $\delta>0$.
\item More importantly, we can simulate the hydrodynamics of particles near a wall or inside a slit channel.
\item A completely different method than \gls{FCM}.
\item Solving the Stokes equation with Gaussian sources (basically ibm-like).
\item Fourier transform only in XY, Chebyshev in Z
\item BCs are incorporated into the solver via corrections. We have free and one or two walls
\item BVP solver
\item Particles overlapping the walls are relfected so their contribution is zero when they are exactly at a wall.
\end{itemize}
...
\end{aligned}
\end{equation}
Where the domain is here defined in the $[-1, 1]$ range for simplicity. We can later generalize to any size via a simple change in units.
...
We will work with the discrete Chebyshev transform of eq. \eqref{eq:bvpmain}. In general, we will expand a given function, $g(z)$ with $z\in \mathcal{R}$ into the first $N$ terms of its Chebyshev series so that
\begin{equation}
\label{eq:bvpchebexp}
g(z) = \sum_{n=0}^{N-1} \fou{g}_n T_n(z)
\end{equation}
Where we use the subscript $n$ to distinghuish between the Chebyshev and Fourier coefficients (since throught this manuscript we have used the subscript $k$ to denote Fourier coefficients).

If we evaluate $f(z)$ in the roots of the Chebyshev polynomials, $z_i=cos(i\pi/N)$, we can leverage the \gls{FFT} to provide the coefficients of the Chebyshev transform (in what is usually refered to as a Fast Chebyshev Transform (FCT)) by using it to compute a discrete cosine transform. The proof, while straight forward, is cumbersome and beyond the scope of this work, it can be found in Chapter 8 of\cite{Trefethen2000}. In particular, the fast Chebyshev transform is carried out by taking the \gls{FFT} of the periodic extension of $f(z)$\footnote{cuFFT can be used for this, which is the approach in \uammd.} (see Appendix \ref{sec:fct}).

On the other hand, we can use the well known indefinite integrals of the Chebyshev polynomials to integrate $\fou{y}''_n$ twice and get $\fou{y}_n$. First by integrating $\fou{y}''_n$:
\begin{equation}
  \begin{aligned}
...
\begin{equation}
  \begin{aligned}
&\hat{y}_1 = \frac{1}{2}\left(2\hat{y}'_0-\hat{y}'_2\right)=\hat{y}'_0 -\frac{1}{8}\left(\hat{y}''_1-\hat{y}''_3\right)\\
&\hat{y}_2 = \frac{1}{4}\left(\hat{y}'_1-\hat{y}'_3\right)=\frac{1}{4}\left(\frac{1}{2}\left(2\hat{y}''_0-\hat{y}''_2\right)-\frac{1}{6}\left(\hat{y}''_2-\hat{y}''_4\right) \right)\\
&\hat{y}_n = \frac{1}{2n}\left(\hat{y}'_{n-1}-\hat{y}'_{n+1}\right)=\frac{1}{2n}\left(\frac{1}{2n-2}\left(\hat{y}''_{n-2}-\hat{y}''_n\right)-\frac{1}{2n+2}\left(\hat{y}''_n-\hat{y}''_{n+2}\right) \right), \quad n \geq 3.
\end{aligned}
\end{equation}
Here we have done this in painstaking detail to point out that there are several exceptions when computing $\hat{y}_n$ from $\hat{y}''_n$. In most Chebyshev series notations, $\hat{y}_0$ is defined in a way that circumvents these exceptions (i.e., with a 1/2 in front of it). We refrain from doing so here to avoid easily (at least for me!) encountered confusion. In addition, note that there are two free parameters $\hat{y}_0$ and $\hat{y}'_0$. We also assume $\hat{y}''_n = \hat{y}'_n = 0$ for $n > N-1$ when calculating $\hat{y}_n$ using\ \eqref{eq:cs}.  

Now, we can reformulate the boundary value problem using the Chebyshev series representations\ \eqref{eq:chebexp} as
\begin{gather}
\label{eq:bvpn0}
\sum_{n=0}^{N-1} (\hat{y}''_n-k^2\hat{y}_n)T_n(z) = \sum_{n=0}^{N-1}\hat{f}_n T_n(z).
\end{gather}
Matching modes gives a system of equations for the Chebyshev coefficients
\begin{gather}
\label{eq:modes}
\hat{y}''_n-k^2\hat{y}_n=\hat{f}_n \qquad n=0, \dots N-1, \\[2 pt]
\label{eq:BCs}
\sum_{n=0}^{N-1} (\hat{y}'_n+k\hat{y}_n) = \alpha, \qquad \sum_{n=0}^{N-1} (\hat{y}'_n-k\hat{y}_n) (-1)^n = \beta.
\end{gather}
with $N+2$ equations and $N+2$ unknowns $\hat{y}_0, \hat{y}'_0, \hat{y}''_0, \dots \hat{y}''_{N-1}$. We solve the algebraic system of equations\ \eqref{eq:modes} and \eqref{eq:BCs} for the second derivative coefficients $\hat{y}''_0, \dots \hat{y}''_{N-1}$ and integration constants $\hat{y}_0$ and $\hat{y}'_0$, then determine $\hat{y}_n$ by applying the double integral operation on the coefficients\ \eqref{eq:cs}.

Now suppose we want to do our solves on $[0,H]$, so that the domain is enlarged by a factor $H/2$. Then each integral operator gets a factor of $H/2$. As such, the equations are
 \begin{gather}
\sum_{n=0}^{N-1} (\hat{y}''_n-k^2\frac{H^2}{4}\hat{y}_n)T_n\left(\frac{2z}{H}-1\right) = \sum_{n=0}^{N-1}\hat{f}_n T_n\left(\frac{2z}{H}-1\right)\\[2 pt]
\hat{y}''_n-k^2\frac{H^2}{4}\hat{y}_n=\hat{f}_n \qquad n=0, \dots N-1, \\[2 pt]
\sum_{n=0}^{N-1} \left(\frac{H}{2}\hat{y}'_n+k\frac{H^2}{4}\hat{y}_n\right) = \alpha, \qquad \sum_{n=0}^{N-1} \left(\frac{H}{2}\hat{y}'_n-k\frac{H^2}{4}\hat{y}_n\right) (-1)^n = \beta.
\end{gather}

For the $k=0$ mode, the system reduces to
 \begin{gather}
\sum_{n=0}^{N-1} \hat{y}''_n T_n\left(\frac{2z}{H}-1\right) = \sum_{n=0}^{N-1}\hat{f}_n T_n\left(\frac{2z}{H}-1\right),
\end{gather}
which gives the trivial system of equations
\begin{gather}
\label{eq:modes0}
\hat{y}''_n=\hat{f}_n \qquad n=0, \dots N-1.
\end{gather}
Our solves for the $k=0$ mode will use the homogeneous boundary conditions
\begin{gather}
\label{eq:BCs0}
\sum_{n=0}^{N-1} \hat{y}_n = 0, \qquad \sum_{n=0}^{N-1} \hat{y}_n (-1)^n = 0. 
\end{gather}

\subsubsection{Matrix inversion}
\label{sec:schurc}
We use a Schur complement approach to solve the algebraic system of equations\ \eqref{eq:modes} and \eqref{eq:BCs}. We can write the system in block form as 
\begin{equation}
\label{eq:blocksys}
\begin{pmatrix} \bm{A} & \bm{B} \\[2 pt] \bm{C} & \bm{D} \end{pmatrix}
\begin{pmatrix} \hat{\bm{y}}''\\ \hat{y}_0 \\ \hat{y}'_0 \\ \end{pmatrix}
= \begin{pmatrix}  \hat{\bm{f}}\\ \alpha \\ \beta\\ \end{pmatrix}. 
\end{equation}
Here $\bm{B}$ is $N \times 2$, $\bm{C}$ is $2 \times N$, $\bm{D}$ is $2 \times 2$, and $\bm{A}$ is an $N \times N$ \textit{pentadiagonal} matrix, which can be inverted using a fast algorithm for such matrices. Therefore, we first invert $\bm{A}$, then solve the $2 \times 2$ system
\begin{equation}
(\bm{C}\bm{A}^{-1}\bm{B}-\bm{D})\begin{pmatrix}  \hat{y}_0 \\ \hat{y}'_0\\ \end{pmatrix} = \bm{C}\bm{A}^{-1}\bm{f}-\begin{pmatrix}  \alpha \\ \beta\\ \end{pmatrix}
\end{equation}
for $\hat{y}_0$ and $\hat{y}'_0$. We then obtain the coefficients $\hat{\bm{y}}''=(\hat{y}''_0, \dots \hat{y}''_{N-1})$ from back-substitution
\begin{equation}
\hat{\bm{y}}''=\bm{A}^{-1}\left(\bm{f}-\bm{B}\begin{pmatrix}   \hat{y}_0 \\ \hat{y}'_0\\ \end{pmatrix}\right).
\end{equation}

\chapter{The Fast Chebyshev Transform}
