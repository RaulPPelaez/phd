...
\section{Linear Bounding Volume Hierarchy neighbour list}\label{sec:lbvh}
...
\item Sorts the particles by assigning a hash to each particle such
  ...
  sorted by type and then by Z-order hash
  ...
  generate an entirely new tree for every type as in \cite{Howard2016}, \cite{Howard2019}.
...
\item The neighbours of a given particle are found by
 ...
 \end{enumerate}
 The traversal is done with a top-down approach, each particle starts at the root of a given subtree. If the particle overlaps the node with a given cut off, the algorithm goes to the next child node, otherwise it skips to the next node/periodic image/tree. The algorithm is detailed in [1] and \cite{Torres2009}.
 ...
 In my personal experience, the sheer raw power of the cell list in the GPU makes this algorithm not worth in general. Note however, that this algorithm is bound to outperform spatially-partitioning methods (such as the cell list and, as a consequence, the Verlet list) in certain situations, mainly when the size disparities between the different particles in the simulation is really pronounced or when the configuration presents a very low density (in terms of the cut off distance of the interaction).

 \subsection*{Usage in UAMMD}

The interface for this neighbour list in UAMMD is more restricted than those of the previously introduced ones. The reason for this being its, yet to be found, applicability in simulations. Nonetheless, it can be used in any place where CellList or VerletList
\begin{code2}[The different ways of using a LBVH List in \uammd. The \emph{Transverser} interface, already used in chapter \ref{sec:nbody}, is described in detail in Appendix \ref{ch:transverser}. Note that the \emph{Transverer} and \emph{NeighbourContainer} options are identical to the case of a \emph{CellList}.]{label=code:lbvhlist}
#include<uammd.cuh>
...
We also note that $\int_{V_p}\vec{f}d\vec{\fpos}^3 = \lambda$ is the total force induced by the particle to the fluid ( induced force \cite{Mazur1974}).
Hence, $m_f\partial_t\vec{\pvel} = \vec{F}_{\text{drag}} + \lambda$, being $m_f$ the mass of the fluid displaced by the particle.
...
\begin{equation}
  m_e\dot{\vec{\fvel}} = \vec{F} - \lambda.
\end{equation}
Which implies, for $m_e = 0$, $\lambda=\vec{F}$. Thus any force on the particle goes to the fluid.
...
We can now write an equation for the displacements (velocities) of a group of particles using eqs. \eqref{eq:bdhivel} and \eqref{eq:greenstokes}.
\begin{equation}
...
\begin{equation}
  \label{eq:rpymobfour}
  \begin{aligned}
    \tens{M}_{ij}^{\textrm{RPY}} &= \frac{\eta^{-1}}{(4\pi a^2)^2} \int_{S_i}dS_i\int_{S_j}dS_j \int d\vec{k} \exp(i\vec{k}\vec{\ppos}_{ij})\hat{\oper{L}}(\vec{k})\\
    &= \eta^{-1} \int \frac{\exp(i\vec{k}\vec{\ppos}_{ij})}{k^2}\left(\sinc(ka) \right)^2\left(\mathbb{I} - \frac{\vec{k}\otimes\vec{k}}{k^2}\right)d\vec{k}
\end{aligned}
\end{equation}
