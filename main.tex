...
\item Fokker-Planck: Particles move so slowly compared to the solvent characteristic times that hydrodynamic interactions are effectively instantaneous. Only the positions and momentum of the particles remain as relevant variables (Lagrangian description once again), which are described by a Langevin equation (the \gls{SDE} equivalent of a \gls{FPE}). The characteristic decorrelation time of the particle's velocities governs this scale with $\tau\sim 10^{-5}$. We explore this inertial level of detail in chapter \ref{sec:langevin}.
...
Going from one level to the next incurs losing information (AKA degrees of freedom, e.g the hydrodynamic level cannot track the positions/momenta of the microscopic solvent particles). These eliminated, fast, degrees of freedom are modeled (reintroduced) as a drag coupled with thermal noise. In future sections, we will see how dissipation is intimately related with thermal noise via the fluctuation-dissipation theorem.
...
Here $\vec{\partial}_{\vec{x}}\cdot \vec{J} := \nabla_{\vec{x}}\cdot\vec{J} = \sum_i\partial J_i/\partial_{x_i}  $ represents the divergence.
...
We can use the probability distribution $P$ to compute the time evolution of the average of a certain function $f(\vec{x}(t))$
...
\end{equation}
of the relevant variables. Here $\otimes$ represents the tensor product.

With this notation, using eqs. \eqref{eq:fpemean} and \eqref{eq:fpevar}, it can be shown that the time evolution of the covariance is given by
%\begin{equation}
%  \label{eq:fpesigma}
%  \partial_t\tens{\sigma} = 2\left\langle\vec{D}^{(2)} \right\rangle + \left\langle \vec{x}\otimes\vec{D}^{(1)} \right\rangle - \left\langle \vec{x} \right\rangle\otimes\left\langle \vec{D}^{(1)}\right\rangle
%\end{equation}
%This allows to rewrite eq. \eqref{eq:fpesigma} as  %, where  $\bar{\vec{x}} := \langle \vec{x} \rangle$.
\begin{equation}
 \partial_t\tens{\sigma} = 2\left\langle\vec{D}^{(2)} \right\rangle + \left\langle \delta\vec{x}\otimes\delta\vec{D}^{(1)} \right\rangle
\end{equation}
For convenience, let us define $\bar{\vec{x}}:=\left\langle\vec{x}_{\text{eq}}\right\rangle$.
...
\begin{equation}
  \delta\vec{D}^{(1)} \approx \vec{partial}_\vec{
\end{equation}
...
%Similarly, we can define $\delta\vec{D}^{(i)} = \vec{D}^{(i)} - \langle \vec{D}^{(i)} \rangle_{\text{eq}}$ for any of the transport coefficients.
\begin{itemize}
