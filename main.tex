...
Developing new techniques (numerical and theoretical) directly aimed to these new technologies will enable the exploration of regimes previously unreachable, opening the doors to a world of secrets waiting to be found.
The fields that can take advantage of more computing power the most are those that mix several spatio-temporal regimes.
For instance, when we need to simulate every molecule of a virus capsid submerged in water for a very long time\cite{virusfullatom2018}. In this article, the authors simulate the dynamics of $6$ million atoms (explicitly) in the span of $1\mu s$, requiring $5\cdot10^8$ simulation steps.
In another example of this kind of simulations, the authors of \cite{fullatombacteria2016} use $100$ million particles to model a chunk of the interior environment of a bacteria for some tens of nanoseconds (around $10^7$ steps).
...
This thesis is devoted to the development of new algorithms and software tools for the study of complex fluids and biological systems in high performance computing environments.

It just so happens that the interesting physics of these processes often lie in this intermediate regime, called the \emph{mesoscale}.
...
One of the main unwritten lessons we will glimpse at while descending the tree is that it is never a good idea to introduce dependencies that climb up through the tree. This is what makes \uammd so modular and extensible. Just to give some examples of this:
\begin{itemize}
...
Modularity is another central philosophy in \uammd. A simulation is described by connecting a series of \emph{Integrators} with an \emph{Integrator}. In fig. \ref{fig:modular} we can see an example of this, in which a hydrodynamics \emph{Integrator} (such as the ones described in sec. \ref{sec:bdhi}) is connected with an \emph{Interactor} that deals with sterics (see sec. \ref{alg:shortrange}), another one that computes electrostatics (see sec. \ref{ch:tppoisson}), and so on and so forth. The communication between these modules is minimal, being mostly restricted to the closed loop in fig. \ref{fig:uammdsketch}. Although nothing prevents two \emph{Interactors} from explicitly sharing information, this never happens in \uammd. 

Even when the internal complexity of each separate module can be quite great it never bleeds into the rest of the modules, which can remain oblivious to each other. In a way, a module will have a lot of internal complexity but rather few external conections. This allows each module to worry only about solving one specific problem (hydrodynamics, electrostatics,...).
...
The modularity of \uammd is reflected directly in the code, where modules can be created independently of each other as in the example below.
Be it an \emph{Integrator} (hydrodynamics, molecular dynamics,...) or \emph{Interactor} (short range, bonded,...), all modules in \uammd can be created and stored as individual shared pointers\footnote{A C++ specific kind of reference. See Appendix \ref{sec:cpp} for basic notions of C++.}. For instance, throughout this manuscript, we will typically create a module named ``Module'' a code similar to the one in code \ref{code:module}.
\begin{code}
\begin{minted}{\ucpp}
#include<uammd.cuh>
...
\end{minted}
  ...
  \label{code:module}
\end{code}
Where ``Module'' will be called, for instance, \emph{VerletNVE} if it is \gls{MD} \emph{Integrator} (see sec. \ref{sec:velocityverlet}) or \emph{BondedForces} if it is a ligature \emph{Interactor} (see sec. \ref{sec:bonds}).

The argument of this function, of type \emph{UAMMD}, is simply an aggregate (a \emph{struct} in C++) of recurrent parameters. In particular, this structure can be defined as follows
\begin{code}
\begin{minted}{\ucpp}
#include<uammd.cuh>
...
In C++, a function is executed in the CPU unless it is explicitly marked as a \gpu one (see sec. \ref{sec:cpp} for more information about CUDA/C++ programming). Accessing \gpu memory in a CPU function is thus illegal and results in the code crashing.
