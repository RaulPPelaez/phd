...
Consider the development of a software capable of reproducing the dynamics of an ionic solution in a cellular membrane channel. This would be a really convoluted simulation, involving electrostatics, fluctuating hydrodynamics, steric interactions, bonded ligatures, etcetera\footnote{A simulation easily handled by \uammd, by the way.}. As we will see during this manuscript, some of these algorithms are quite complex on their own, depending on a lot of moving parts. The mathematical and theoretical machinery required to model something like that appears to be in the realms of a theory of everything. It surely seems like an intractable feat!. We could still try to design, from the ground up, a software that can perform this hypothetical simulation. The result would even be somewhat flexible. For instance, we could turn off electrostatics by setting the charges of all particles to zero.
This top-down approach is fundamentally flawed, we are trying to build the house starting by the roof and losing sight of the forest in the process. The final product will probably have a lot of internal dependencies, many of which are probably unnecesary. In software this means that the code will be hard to adapt or extend.
...
This approach starts by dividing the original problem into subproblems in a hierarchical fashion (see fig. \ref{fig:uammdsketch}). So instead of designing our framework with an ionic solution in a membrane channel in mind, we start at the base and then hierarchically especialize.
We can do this by removing assumptions until we get to the root of the problem. Therefore, we do not design in terms of ``ions'', ``cells'' or ``molecules'', rather in terms of ``particles''.
...
The \uammd framework takes this KISS\footnote{Keep It Simple, Sir} philosophy very seriously. It is supported on a handful of vague assumptions which can be summarize in four:
\begin{enumerate}
\item The code will run primarily on a \gpu (the most limiting assumption).
\item Simulations are based on the state of ``particles'' (whatever a particle means).
\item The state of these particles changes in time.
\item Particles can interact between them and with an external influence.
\end{enumerate}
...
Then, we can focus on each of the last three asumptions and apply this separation again. For instance, instead of grouping all interactions (fourth assumption) into a single entity (class) that deals with all possible interactions, we can split the concept between general things like short ranged, bonded or long ranged interactions.

During the following chapters, we will discuss in detail this conceptual and programmatic hierarchy and follow its branches until the leafs. This final objects will encapsulate the actual algorithms and physics in the form of separated, individual modules that will in general communicate only upwards in the hierarchy.

\chapter{It's in the name}\label{ch:design}

It is standard knowledge that a good software starts with a good acronym. \uammd stands for Universally Adaptable Multiscale Molecular Dynamics, lets see what each of these words stands for.
\section{Universality}
% Anything goes as long as it has interacting entities. One could technically implement Chess as a module.
\begin{itemize}
\item Multiphysics
\item Eulerian Lagrangian descriptions.
\item You can use a particle-based description with interactions or add a fluid also. Or just a fluid
\item The limit would be only classical physics
\end{itemize}
...
\begin{figure}
  \centering
  %\includegraphics[width=\textwidth]{landscape}
  \includesvg{gfx/sketchUAMMD}{\columnwidth}
  \caption{The basic hierarchy of concepts (and code) in \uammd. \emph{System} holds information about the actual physical hardware, and all the entities below rely on it to interact with the environment. \emph{ParticleData} stores all the information about the particles in the simulation, such as positions ($\vec{r}$), velocities ($\vec{v}$), mass, etc. And other properties like the current forces acting on it ($\vec{F}$), energy ($E$) or virial ($T$). The modules below it can request, at their leisure, a list with any of the particles properties. Finally, \emph{Integrators} and \emph{Interactors} are interfaces, called virtual classes in programming terms, that are in charge of, respectively, forwarding the simulation one step in time and computing the forces, energies and/or virials acting on each particle. Interfaces, drawn in red, are abstract objects that cannot be instanced on their own but rather must be inherited. For instance, Brownian Dynamics (section. \ref{sec:bd}) would be an \emph{Integrator}, while a module that computes gravitational forces would be an \emph{Interactor}.}
...
\item Open/templated (logic can come from outside)
\item UAMMD as a library (can be used as an external tool)
\item Explain the concept of module in \uammd. An entity that has much more internal connections than external connections.
\item Draw a box, with only two connections (an input and an output) and a lot of internal complexity.
\item With boxes draw an skematich of a simulation (an MD box + an Interactor box gives a simulation)
...
\item Open infrastructure instead of closed ecosystem. Open source. C++, header only.
\item UAMMD shpould be understood not as a stand alone code, but as a library/framework.

\end{itemize}
\section{Mulstiscale}
From atoms in vacuum to chunks of oceans and planetary dynamics.
\begin{itemize}
\item Image of multiscale
...
\begin{itemize}
\item Particles are the important entity. If a fluid is present it is only a medium to move the particles.
\item molecular dynamics understood as ``dynamics of molecules'' .
\item Interacting particles.
\item Lagrangian or Eulerian-Lagrangian methods.
\item Interactions in \uammd always act on particles (particle-particle, particle-fluid...). Introduce the concept of Interactor.
...
\begin{figure}
  \centering
  %\includegraphics[width=\textwidth]{landscape}
  \includesvg{gfx/scales}{\columnwidth}
...
\todo{Fusilarle la figura de la multiscala a Rafa}
...
Let us interpret the word \emph{particle} as a relatively small discrete portion in a simulation. This is a vage definition on porpuse, since from an algorithmic point of view it does not really matter what a particle represents. Sometimes a particle will be an atom or molecule, other times it will make more sense that our simulation unit is a big colloid, a virus or a whole sun.\todo{This paragraph should go to the Design principles-> molecular dynamics section}
...
It is also worth introducing here two other interfaces that \uammd uses to work with particles, \emph{Transverser} and \emph{Potential}.\todo{These should go when they are needed, after explaining some force module}
\todo{Transverser should go to an Appendix}
...
\section{Periodic Boundary Conditions} \todo{This goes in a section called Domain in UAMMD. Explain periodic vs open etc.}
...
\end{minted}
\todo{Mention the NeighbourCountar examples, gives N for every particle}
...
The divergence of the mobility in the last term, commonly referred to as thermal drift, is an stochastic term stemming from the Itô interpretation of the Brownian equation and its necessary for time reversibility (detailed balance) of the limiting dynamics.\todo{Referencias, me gustaria explicar mejor este termino}
...
\begin{figure}[h]
  \centering
  \includesvg{gfx/q2d}{\columnwidth}
...
\begin{figure}[h]
  \centering
  \includesvg{gfx/dpstokesspread}{\columnwidth}
