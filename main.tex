...
In order to do this we first spread particle forces to the fluid ($\oper{S}\vec{F}$ in eq. \eqref{eq:bdhibdrelation}) and transform them to Fourier space. Then the Fourier representation of the Stokes solution operator in eq. \eqref{eq:stokesoperfou} is used to obtain the velocities in the fluid in Fourier space via multiplication. The fluid velocity is then transformed back to real space and eq. \eqref{eq:bdhifvel} is applied to get the particle displacements by interpolation.
The kernel used in \gls{FCM} is a Gaussian, in particular
\begin{equation}
  \label{eq:fcmkernel}
...
\begin{equation}
  \fou{\vec{u}}(\vec{k}) = \frac{1}{\eta k^2}\left(\mathbb{I} - \frac{\vec{k}\otimes\vec{k}}{k^2}\right)\left( i\vec{k}\cdot\fou{\mathcal{Z}} + \fou{\vec{f}}\right)
\end{equation}
...
\begin{equation}
  \label{eq:pserpyfou}
  \tens{M}_{ij}^{\textrm{RPY}}= \frac{1}{\eta V} \sum_{\vec{k}} \frac{\exp(i\vec{k}\vec{r})}{k^2}\left(sinc(ka) \right)^2\left(\mathbb{I} - \frac{\vec{k}\otimes\vec{k}}{k^2}\right)
\end{equation}
...
\begin{equation}
  \tens{M}_{ij} = \tens{M}_{ij}^{\far} + \tens{M}^{ \near}_{ij}
\end{equation}
...
\end{equation}
Here $\xi$ is the splitting parameter (see sec. \ref{ch:tppoisson}), an arbitrary parameter that allows to shift the weight of the mobility from one term to another.
The integral for the inverse transform of the near part can be computed analytically, this term can be written as
\begin{equation}
  \label{eq:pserpynear}
  \tens{M}_{ij}^{\near}= F(k,\xi)\mathcal I - G(k,\xi)\left(\frac{\vec{r}\otimes\vec{r}}{r^2}\right)
\end{equation}
Where the functions $F$ and $G$ have really convoluted expressions that can be found in Appendix A of\cite{Fiore2017} or in the \uammd source code\footnote{This expressions are really long and convoluted. The corresponding code evaluating the near field in eq. \eqref{eq:pserpynear} is located in the source file \emph{RPY\_PSE.cuh}, which is written to facilitate its copy-pasting for other implementations.}.
...
This allows to use the techniques we have already used in previous sections to evaluate each term. The deterministic term (the multiplication of the mobility by the forces acting on the particles) can simply be computed separatedly and then added. The fluctuations pose more of a challenge if we want to refrain from computing storing the square root of the mobility. In particular, we can separate the fluctuating contribution in eq. \eqref{eq:bdhi} in a far and near field ones by defining
\begin{equation}
...
The far field computation is thus identical to the \gls{FCM} if we use $\sigma_a = ...$ as the width of the kernel (or equivalently, setting the hydrodynamic radius to $a=...$)\todo{complete}. And also differences in the mobility (a factor in the noise)
For the near field, we can use the infrastructure in the \emph{Lanczos} algorithm (see sec. \ref{sec:lanczos}) with the mobility in eq. \eqref{eq:pserpynear}.
