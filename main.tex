...
\begin{equation}
  \vec{F} = \partial_{\vec{q}}U(\vec{q}) + \vec{F}^{\ext}(\vec{q})
\end{equation}
\todo{Rafa, is this ok?}
...
\begin{equation}
  \rho\int_V_p\partial_t\vec{\fvel}d\vec{\fpos}^3 0 -\int_V_p\nabla\cdot\tens{\sigma}d\vec{r}^3 + \int_V\vec{f}d\vec{\fpos}^3
\end{equation}
Note that
$$\rho\int_V\partial\vec{\fvel}d\vec{\fpos}^3 = \rho \partial_t\int_V_p\vec{\fvel}d\vec{\fpos}^3 = \rho\partial_t\vec{\fvel}$$
And
$$
...
We also note that $\int_V_p\vec{f}d\vec{\fpos}^3 = \lamba$ is the total force induced by the particle to the fluid ( induced force \cite{Mazur1974}).
Hence, $m_f\partial_t\vec{\pvel} = \vec{F}_{\text{drag}} + \lambda$, being $m_f$ the mass of the fluid displaced by the particle.
On the ther hand, $m_p
