...
\begin{equation}
  \fou{\nabla s_{\vec{i}}}^\alpha = \frac{\exp(-i\pi k_{\alpha} h)}{h}(\fou{s}_{\vec{k}} - \exp(i2\pi k_\alpha h)\fou{s}_{\vec{k}}) = \frac{2i
\end{equation}

So by taking the Fourier transform of eq. \eqref{eq:staggeredgrad}
\begin{equation}
  (\fou{\nabla} s)^\alpha_{\vec{k}} = \frac{1}{h}(\fou{s}_{\vec{k}} - \mathfrak{F}(s_{\vec{i}- \vec{\hat{\alpha}}})) = \frac{1}{h}(s_{\vec{k}} - \exp()
\end{equation}
