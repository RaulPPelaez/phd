...
In some way or another, all algorithms must compute forces between particles.
\chapter{Working with particles}

\chapter{Computing forces}
We can distinguish between several different types of particle interaction potentials, mainly:
\begin{itemize}
...
In this case, if we want to compute the force acting on a star due to the presence of all the others we must check each and everyone of them. This leads to a lot of interactions to check, more precisely $N^2$ of them. Of course, there are more sophisticated ways of performing such a computation, such as fast multipole methods \cit{fmm} or Ewald splitting (which will be discussed on section \ref{ewald}). But some times these techniques are not a possibility so it is valuable to see how to efficiently handle this computations.
In particular this computation is a really good fit for a \gls{GPU} as we are going to see.
\subsection{The NBody algorithm}
This algorithm is based on the \emph{particles} algorithm originally devised by NVIDIA\cite{nbody1,nbody2}. It leverages the shared memory capabilities of the \gls{GPU} by assigning a thread to a particle and then collaboratively loading groups of particles into shared memory. Then each thread processes this group of particles loaded into shared memory. This is repeated until all particles have been processed.

\begin{algorithm}
  \caption{NBody algorithm GPU kernel}
  \begin{algorithmic}[1]
    \Function{Nbody}{list of positions, box}
    \State \Requires A thread is launched per particle
