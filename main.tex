...
\begin{eqnarray}
  ...
  \vec{r}^{n-1} &=& \vec{r}^n - \vec{v}^n\dt + \half\vec{a}^n\dt^2 - \vec{b}^n\dt^3 + O(\dt^4)
\end{eqnarray}
Where $\vec{b}$ is just the term accompanying the $O(\dt^3)$ term.
...
The dissipative or drag term opposes the movement of the particles and is often written as $\vec{F^d} = -\xi \vec{v}$. The friction coefficient, $\xi$, is related with the damping rate as $\gamma = \xi/m$, which represents the decorrelation time of the velocity, $\tau = m/\xi$. Additionally, $\xi$ will depend on the properties of the solvent, i.e. viscosity, and the submerged particle, i.e. radius.
...
\begin{equation}
  ...
  m\vec{a} = \vec{F^c} - \xi \vec{v} + \sqrt{2\xi k_BT}\vec{\widetilde{W}}
\end{equation}
...
\todo{
  Riemman integral of W can be approximated.
  Gronbech Jensen description from paper.
  No stochastic calculus yet, wait for BD.
}
...
\todo{Stochastic calculus, Riemman approx to integral of brownian, ito integral.}
